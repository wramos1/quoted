...
const Posts = () => {
    ...
    const { uploadNewPost } = useStorage();
    ...
    async function handleSubmit(e) {
        ...
        if (quoteRef.current.value.trim() === '') {
            alert("Please provide quote");
            return;
        }
    ...
    return (
        <section className='flex flex-col justify-center items-center min-h-screen auth-bg text-black p-8'>
            <div className="form-container w-[375px] relative border bg-white flex justify-center flex-col text-center rounded py-10">
                <h2 className='text-[2.5em]'>New Post</h2>
                <form onSubmit={handleSubmit} className='flex justify-evenly flex-col h-3/4 gap-4 p-[10px]'>
                    ...
                    <div className='flex flex-col text-left'>
                        ...
                        <input className='text-black py-[10px] px-[5px] text-[1.1em] bg-zinc-300 rounded' type='text' id='author' maxLength={12} required ref={authorName} />
                    </div>
                    <div className='flex flex-col text-left'>
                        <label htmlFor="quote">Quote</label>
                        <textarea className='text-sm text-black py-[10px] px-[5px] text-[1.1em] bg-zinc-300 rounded' rows={5} type='text' id='quote' required ref={quoteRef} />
                    </div>
                    <button
                        disabled={loading}
                        type='submit'
                ...
                <Link href={'/'} className='absolute bottom-0 text-center w-full text-zinc-500 hover:text-black'>
                    Go to Dashboard
                </Link>
            </div>
        </section>
